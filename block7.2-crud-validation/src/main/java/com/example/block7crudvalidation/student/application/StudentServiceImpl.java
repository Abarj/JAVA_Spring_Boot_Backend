...
public class StudentServiceImpl implements StudentService {
    ...
    public StudentOutputDTO addStudent(StudentInputDTO studentInputDTO) {
        ...
        List<Subject> subjects = getSubjectsIds(studentInputDTO.getSubjects());
    ...
    @Override
    public StudentOutputDTO addSubjects(String idStudent, List<String> subjectsInsert) {
        Student student = studentRepository.findById(idStudent)
                .orElseThrow(() -> new NotFoundException("No student found with id: " + idStudent));
        List<String> idsSubjects = student.getSubjects().stream().map(Subject::getIdSubject)
    ...
    private List<Subject> getSubjectsIds(List<String> ids) {
        List<Subject> subjects = new ArrayList<>();
        if (ids != null) {
            subjects = subjectRepository.findAllById(ids);
            if (ids.size() != subjects.size()) {
                throw new NotFoundException("No se han encontrado asignaturas con el ID: " + ids);
            }
        }
    ...
    public Student getStudentId(String id) {
        return studentRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("No se ha encontrado al estudiante con el ID: " + id));
    }
    ...
    public StudentOutputDTO updateStudent(String id, StudentInputDTO studentInputDTO) {
        ...
        Person person = personRepository.findById(studentInputDTO.getIdPerson())
                .orElseThrow(() -> new NotFoundException("No se ha encontrado a la persona con el ID: " + id));
    ...
    @Override
    public StudentOutputDTO deleteSubjects(String idStudent, List<String> subjectsDelete) {
        Student student = studentRepository.findById(idStudent)
                .orElseThrow(() -> new NotFoundException("No se ha encontrado al estudiante con el ID: " + idStudent));
